...
### Funcionalidade que calcula o preço médio de uma pessoa que faz percurso UM Braga - UM Guimarães
...
def chooseCars():
    ...
    while True:
        ...
        if (new_fuel == 'Não'):
            print(result[['Marca','Modelo','Combustível','Circuito','Gasto']])
            return(result)
...
plt.ylabel('Gasto')
plt.title('Gasto dos Carros por Modelo e Tipo de Combustível na Estrada Nacional')
...
plt.xlabel('Modelo do Carro')
plt.ylabel('Gasto')
plt.title('Gasto dos Carros por Modelo e Tipo de Combustível na Auto-Estrada')
plt.legend(title='Tipo de Combustível', bbox_to_anchor=(1.05, 1), loc='upper left')
plt.xticks(rotation=30)
